...
const FEATURES = {
  free: [
    ...
    "Browse all subjects (read-only previews)",
    "Focus Mode timer",
    { neg: true, text: "No past papers" },
    { neg: true, text: "No exam-board papers" },
  ],
  basic: [
    "30 AI generations per day",
    "All subjects unlocked",
    "Generic mock practice papers",
    "Flashcards & quizzes",
    "AI tutor chat",
    { neg: true, text: "No exam-board mapped papers" },
  ],
  standard: [
    "Unlimited AI generations",
    "All subjects unlocked",
    "Full practice papers + PDF export",
    "Unlimited AI tutor",
  ...
  pro: [
    ...
    "Exam-board papers (AQA, Edexcel, OCR, IB, CIE)",
    "GCSE / A-Level / IB style papers",
    "Priority support",
    "Early access to new features",
  ],
  school: [
    "Whole-school annual licence",
...
const ICONS = { free: Sparkle, basic: GraduationCap, standard: Sparkle, pro: Crown, school: Buildings };
...
export default function Pricing() {
  ...
  const orderedIds = ["free", "basic", "standard", "pro", "school"];
  const ordered = orderedIds.map((id) => plans.find((p) => p.id === id)).filter(Boolean);
  ...
  const inner = (
    <div className="max-w-6xl mx-auto space-y-10">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <p className="text-[#4A4A4A] mt-4">Cancel anytime. Schools pay annually — one price for everyone.</p>
        {billing && (
          <div className="mt-6 inline-block brutal-card px-4 py-2 bg-butter text-sm">
      ...
      <div className="grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 gap-4">
        {ordered.map((p) => {
          ...
          const accent = {
            free: "bg-white", basic: "bg-mint", standard: "bg-butter", pro: "bg-lavender", school: "bg-peach",
          }[p.id];
          return (
            <div key={p.id} className={`brutal-card p-6 flex flex-col ${accent}`} data-testid={`plan-card-${p.id}`}>
              ...
              <div className="mb-1">
                <span className="font-display font-black text-4xl">£{p.amount.toFixed(p.amount % 1 === 0 ? 0 : 2)}</span>
              ...
              <div className="text-xs uppercase tracking-[0.2em] font-bold text-[#4A4A4A] mb-4">
                {p.id === "school" ? "Per school, unlimited students" : "Per student"}
              </div>
                ...
                {FEATURES[p.id]?.map((f, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-start gap-2">
                      {neg
                        ? <XCircle size={16} weight="bold" className="mt-0.5 text-[#4A4A4A] shrink-0" />
              ...
              {isCurrent ? (
                <button disabled className="brutal-btn bg-white opacity-80 cursor-not-allowed" data-testid={`plan-current-${p.id}`}>
                  Current plan
                </button>
              ) : isFree ? (
      ...
      <p className="text-xs text-[#4A4A4A] text-center max-w-2xl mx-auto">
        Prices in GBP. School annual plan includes unlimited students on a single school licence. Test mode — no real charges on the demo.
      </p>
    </div>
  );
